...
public class IntentRecognitionService
{
    ...
    public IntentRecognitionService(DeepSeekService deepSeekService)
    {
        _deepSeekService = deepSeekService;
    }

    public async Task<RecognizedIntent> RecognizeIntentAsync(string userMessage)
    {
        try
        {
    ...
    private string CreateIntentRecognitionPrompt(string userMessage)
    {
        ...
        var tomorrow = DateTime.UtcNow.AddDays(1).ToString("yyyy-MM-ddTHH:mm:ss");
        ...
        return $$"""
            ...
            POSSIBLE ACTIONS:
            - create_reservation - rent a parking space (keywords: "rent", "book", "reserve", "need parking", "parking spot")
            - cancel_reservation - cancel rental (keywords: "cancel", "cancellation", "don't need", "won't make it")
            - check_availability - check availability (keywords: "available", "free spots", "any spaces", "check")
            - extend_reservation - extend rental (keywords: "extend", "longer", "more time")
            ...
            - reservation_id: reservation number (for cancellation/extension)
            ...
            5. If action is not recognized - isValid = false
            6. For parking, ADDRESS and LICENSE PLATE are crucial
    ...
    private RecognizedIntent ParseIntentResponse(string jsonResponse, string originalMessage)
    {
        try
        {
            var cleanJson = ExtractJsonFromResponse(jsonResponse);
            
            var intent = JsonSerializer.Deserialize<RecognizedIntent>(cleanJson, new JsonSerializerOptions
            {
    ...
    private bool ValidateIntent(RecognizedIntent intent)
    {
        return intent.Action switch
        {
            "create_reservation" => ValidateCreateReservation(intent),
            "cancel_reservation" => ValidateCancelReservation(intent),
            "check_availability" => ValidateCheckAvailability(intent),
            "extend_reservation" => ValidateExtendReservation(intent),
            "get_price" => ValidateGetPrice(intent),
            _ => false
        };
    }

    private bool ValidateCreateReservation(RecognizedIntent intent)
    {
    ...
    private bool ValidateCancelReservation(RecognizedIntent intent)
    {
        return intent.Parameters.ContainsKey("reservation_id") ||
               (intent.Parameters.ContainsKey("address") && 
                intent.Parameters.ContainsKey("start_datetime") && 
                intent.Parameters.ContainsKey("car_number"));
    }

    private bool ValidateCheckAvailability(RecognizedIntent intent)
    {
        return intent.Parameters.ContainsKey("start_datetime") && 
               intent.Parameters.ContainsKey("end_datetime") &&
               intent.Parameters.ContainsKey("address");
    ...
    private bool ValidateExtendReservation(RecognizedIntent intent)
    {
        return intent.Parameters.ContainsKey("reservation_id") && 
               intent.Parameters.ContainsKey("end_datetime");
    }

    private bool ValidateGetPrice(RecognizedIntent intent)
    {
        return intent.Parameters.ContainsKey("start_datetime") && 
               intent.Parameters.ContainsKey("end_datetime") &&
               intent.Parameters.ContainsKey("address");
    ...
    private RecognizedIntent CreateFallbackIntent(string originalMessage)
    {
